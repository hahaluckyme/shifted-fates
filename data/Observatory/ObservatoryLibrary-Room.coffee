...
TechnologicalAdvances: =>
    if CrystalSpectrometer.player_checked_device = true
        ...
        say paragraph "After a couple minutes, you happen upon something promising. The device doesn't look exactly the same, but it's close enough to seem like they share an underlying function. A header sensationally proclaims, \"How spectroscopy changed astronomy forever\". Reading on, you find out that the instrument you saw earlier is actually called a \"spectrometer\", and one of its purposes appears to be determining the exact chemical composition of a material. The instrument accomplishes this by splitting light into component wavelengths and casting those bands onto a detector plate, whereupon the intensity of frequencies is recorded and run against a database. Using this approach, astronomers are able to analyze the chemical composition of any observable object."
        say paragraph "The section continues on about the various different forms of spectrums, as well as detailing how to create emission and absorption spectrums. Once finished reading, you return the book to its spot on the shelf and resolve to take another look at the spectrometer in the research lab."
    else
